...
#include "cyber/transport/rtps/PublisherAttributes.hpp"
#include "cyber/transport/rtps/SubscriberAttributes.hpp"
...
bool GetDefaultPubAttributes(
  ...
  pub_attr->times.heartbeat_period.seconds = 0;
  pub_attr->times.heartbeat_period.fraction(300 * 4294967);  // 300ms
  if (qos.mps() != 0) {
...
bool GetDefaultTopicAttributes(
  ...
  topic_attr->topicKind = eprosima::fastdds::rtps::NO_KEY;
...
bool QosFiller::FillInReaderQos(
  ...
  reader_qos->expects_inline_qos(sub_attr.expects_inline_qos);
  ...
  reader_qos->type_consistency() = sub_attr.qos.type_consistency;
  reader_qos->representation() = sub_attr.qos.representation;
