...
void TopologyManager::OnParticipantChange(eprosima::fastdds::rtps::ParticipantDiscoveryStatus status,
    const eprosima::fastdds::rtps::ParticipantBuiltinTopicData& info) {
  ChangeMsg msg;
  if (!Convert(status, info, &msg)) {
    return;
  }
...
bool TopologyManager::Convert(eprosima::fastdds::rtps::ParticipantDiscoveryStatus status,
    const eprosima::fastdds::rtps::ParticipantBuiltinTopicData& info,
    ChangeMsg* msg) {
  auto guid = info.guid;
  std::string participant_name("");
